...
read USERNAME
USER_DETAIL=$($PSQL "SELECT * FROM users WHERE username = '$USERNAME'")
...
START_GAME(){
    ...
    then
      ...
      then
        ...
        # update games played
        UPDATE_GAME_PLAYED=$($PSQL "UPDATE users SET games_played = games_played +1 WHERE username = '$USERNAME'") 
        
        echo $USER_DETAIL| while IFS='|' read NAME GAME_PLAYED BEST_NO_GUESS
        do
          # if best_no_guess not found or lower than previous best
          if [[ -z $BEST_NO_GUESS || $NO_OF_GUESSES -lt $BEST_NO_GUESS  ]]
          then
             UPDATE_BEST_GAME=$($PSQL "UPDATE users SET best_game = $NO_OF_GUESSES WHERE username = '$USERNAME'") 
          fi
        done
